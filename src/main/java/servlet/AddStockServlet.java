...
public class AddStockServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean success = dao.addStock(itemId, quantity, referenceNote);

        if (success) {
            response.sendRedirect("ViewStockServlet?msg=stock_added");
        } else {
            response.sendRedirect("ViewStockServlet?err=add_failed");
        }
    }
}